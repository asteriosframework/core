...
namespace Asterios\Core\Athene\Traits\Validations;

trait CheckEmailAddress
{
    // Source: https://gist.github.com/jefferyrdavis/5992247
     ...
     */
    public function isEmail($email, $checkDNS = false, $errorlevel = false)
    {
...
// revision 2.5: Undefined unused constants
        ...
        if ($emailLength > 254) if ($diagnose)
            return ISEMAIL_TOOLONG;
        else
        ...
        if ($atIndex === false) if ($diagnose)
            return ISEMAIL_NOAT;
        else
            return false; // No at-sign
        if ($atIndex === 0) if ($diagnose)
            return ISEMAIL_NOLOCALPART;
        else
            return false; // No local part
        if ($atIndex === $emailLength - 1) if ($diagnose)
            return ISEMAIL_NODOMAIN;
        else
        ...
        for ($i = 0; $i < $emailLength; ++$i) {
            ...
            if ($char === '\\')
                ...
                switch ($char) {
                    ...
                    case '.':
                        if ($escapeThisChar)
                            $replaceChar = true; // Dots don't help us either
                        else if ($braceDepth > 0)
                            $replaceChar = true;
...
// revision 1.12: Line above replaced because PHPLint doesn't like that syntax
                if ($replaceChar)
                    $email = (string) substr_replace($email, 'x', $i, 1); // Replace the offending character with something harmless
            }
        }
        ...
        foreach ($dotArray as $arrayMember) {
            ...
            if ($warn && ($element !== $new_element))
                $return_status = ISEMAIL_FWS; // FWS is unlikely in the real world
            $element = $new_element;
            // version 2.3: Warning condition added
            $elementLength = strlen($element);

            if ($elementLength === 0) if ($diagnose)
                return ISEMAIL_ZEROLENGTHELEMENT;
            else
                return false; // Can't have empty element (consecutive dots or dots at the start or end)
...
// version 2.0: Warning condition added
            ...
            if (preg_match('/^"(?:.)*"$/s', $element) > 0) {
                // Quoted-string tests:
                if ($warn)
                    $return_status = ISEMAIL_QUOTEDSTRING; // Quoted string is unlikely in the real world
// version 2.0: Warning condition added
                // Remove any FWS
                ...
                if (preg_match('/(?<!\\\\|^)["\\r\\n\\x00](?!$)|\\\\"$|""/', $element) > 0) if ($diagnose)
                    return ISEMAIL_UNESCAPEDDELIM;
                else
                    return false; // ", CR, LF and NUL must be escaped
// version 2.0: allow ""@example.com because it's technically valid
                ...
                if ($element === '') if ($diagnose)
                    return ISEMAIL_EMPTYELEMENT;
                else
                ...
                if (preg_match('/[\\x00-\\x20\\(\\)<>\\[\\]:;@\\\\,\\."]/', $element) > 0) if ($diagnose)
                    return ISEMAIL_UNESCAPEDSPECIAL;
                else
                    return false; // These characters must be in a quoted string
                if ($warn && (preg_match('/^\\w+/', $element) === 0))
                    $return_status = ISEMAIL_UNLIKELYINITIAL; // First character is an odd one
            }
        }

        if ($partLength > 64) if ($diagnose)
            return ISEMAIL_LOCALTOOLONG;
        else
        ...
        if (preg_match('/^\\[(.)+]$/', $domain) === 1) {
            ...
            if ($warn)
                $return_status = ISEMAIL_ADDRESSLITERAL; // Quoted string is unlikely in the real world
// version 2.0: Warning condition added
...
// revision 2.1: new IPv6 testing strategy
                    if (substr($addressLiteral, 0, 5) !== 'IPv6:') if ($diagnose)
                        return ISEMAIL_IPV6BADPREFIXMIXED;
                    else
...
// revision 2.1: new IPv6 testing strategy
            ...
            if ($index === false) {
                ...
                if ($groupCount !== $groupMax) if ($diagnose)
                    return ISEMAIL_IPV6GROUPCOUNT;
                else
                    return false; // RFC5321 section 4.1.3
            } else {
                if ($index !== strrpos($IPv6, $double_colon)) if ($diagnose)
                    return ISEMAIL_IPV6DOUBLEDOUBLECOLON;
                else
                    return false; // More than one '::'
                if ($index === 0 || $index === (strlen($IPv6) - 2))
                    $groupMax++; // RFC 4291 allows :: at the start or end of an address with 7 other groups in addition
                if ($groupCount > $groupMax) if ($diagnose)
                    return ISEMAIL_IPV6TOOMANYGROUPS;
                else
                    return false; // Too many IPv6 groups in address
                if ($groupCount === $groupMax)
                    $return_status = ISEMAIL_SINGLEGROUPELISION; // Eliding a single group with :: is deprecated by RFCs 5321 & 5952
            }

            // Check for single : at start and end of address
            // Revision 2.7: Daniel Marschall's new IPv6 testing strategy
            if ((substr($IPv6, 0, 1) === $colon) && (substr($IPv6, 1, 1) !== $colon)) if ($diagnose)
                return ISEMAIL_IPV6SINGLECOLONSTART;
            else
                return false; // Address starts with a single colon
            if ((substr($IPv6, -1) === $colon) && (substr($IPv6, -2, 1) !== $colon)) if ($diagnose)
                return ISEMAIL_IPV6SINGLECOLONEND;
            else
                return false; // Address ends with a single colon

            // Check for unmatched characters
            if (count(preg_grep('/^[0-9A-Fa-f]{0,4}$/', $matchesIP, PREG_GREP_INVERT)) !== 0) if ($diagnose)
                return ISEMAIL_IPV6BADCHAR;
            else
...
// revision 1.13: Line above added because PHPLint now checks for Definitely Assigned Variables
            ...
            if ($warn && (count($dotArray) === 1))
                $return_status = ISEMAIL_TLD; // The mail host probably isn't a TLD
// version 2.0: downgraded to a warning
            ...
            foreach ($dotArray as $arrayMember) {
                ...
                // local part, or two periods together. Either way it's not allowed.
                if ($elementLength === 0) if ($diagnose)
                    return ISEMAIL_DOMAINEMPTYELEMENT;
                else
                    return false; // Dots in wrong place
// revision 1.15: Speed up the test and get rid of "uninitialized string offset" notices from PHP
...
// revision 1.17: Fixed name of constant (also spotted by turboflash - thanks!)
                ...
                if ($elementLength > 63) if ($diagnose)
                    return ISEMAIL_DOMAINELEMENTTOOLONG;
                else
                ...
                // Any excluded characters? i.e. 0x00-0x20, (, ), <, >, [, ], :, ;, @, \, comma, period, "
                if (preg_match('/[\\x00-\\x20\\(\\)<>\\[\\]:;@\\\\,\\."]|^-|-$/', $element) > 0) if ($diagnose)
                    return ISEMAIL_DOMAINBADCHAR;
                else
                    return false; // Illegal character in domain name
            }

            if ($partLength > 255) if ($diagnose)
                return ISEMAIL_DOMAINTOOLONG;
            else
                return false; // Domain part must be 255 characters or less (http://tools.ietf.org/html/rfc1123#section-6.1.3.5)

            if ($warn && (preg_match('/^[0-9]+$/', $element) > 0))
                $return_status = ISEMAIL_TLDNUMERIC; // TLD probably isn't all-numeric (http://www.apps.ietf.org/rfc/rfc3696.html#sec-2)
// version 2.0: Downgraded to a warning
            ...
            if ($diagnose && ($return_status === ISEMAIL_VALID) && $checkDNS && function_exists('checkdnsrr')) {
                ...
                if (!(checkdnsrr($domain, 'MX'))) {
                    ...
                    if ((is_bool($result) && !(bool) $result))
                        ...
                        $return_status = ISEMAIL_MXNOTFOUND; // MX-record for domain can't be found
                }
            }
        }
